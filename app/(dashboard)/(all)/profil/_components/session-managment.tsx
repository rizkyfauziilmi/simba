...
export default function SessionManagment() {
  ...
  const { data: session, refetch: refetchCurrentSession } = authClient.useSession()
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Lihat dan kelola semua sesi aktif Anda</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Current Session */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-foreground">Sesi Saat Ini</h3>
            {sessions
              .filter(s => s.id === currentSession.id)
              .map(session => {
                const deviceInfo = parseUserAgent(session.userAgent ?? undefined)
                  ...
                  >
                    <div className="flex gap-4 flex-1">
                      <div className="text-muted-foreground mt-1 space-y-1">
                        ...
                        {getBrowserIcon(deviceInfo.browser ?? 'Unknown Browser')}
                      </div>
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          IP: {session.ipAddress} • {formatDateToNow(session.updatedAt)}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          {sessions.filter(s => s.id !== currentSession.id).length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-foreground">Sesi Lainnya</h3>
                {sessions.filter(s => s.id !== currentSession.id).length > 0 && (
                  <Button variant="destructive" size="sm" onClick={handleRevokeAllOtherSessions}>
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium text-foreground">💡 Tips Keamanan:</p>
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>• Cabut sesi dari perangkat yang tidak Anda kenal</li>
              <li>• Periksa secara berkala untuk aktivitas yang mencurigakan</li>
        ...
        <AlertDialog open={showDialog} onOpenChange={setShowDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Anda akan keluar dari {deviceInfoSelected?.os} ({deviceInfoSelected?.browser}). Anda
                perlu masuk kembali untuk menggunakan perangkat ini.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Batal</AlertDialogCancel>
              <AlertDialogAction onClick={confirmRevoke}>Cabut Sesi</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      )}
      ...
      <AlertDialog open={showRevokeAllDialog} onOpenChange={setShowRevokeAllDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Anda akan keluar dari semua perangkat lain kecuali yang sedang Anda gunakan sekarang.
              Anda perlu masuk kembali pada perangkat tersebut.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Batal</AlertDialogCancel>
