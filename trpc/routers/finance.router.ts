...
export const financeRouter = createTRPCRouter({
  getFinanceSummary: adminProcedure.input(getFinanceSummarySchema).query(async ({ ctx, input }) => {
    ...
    for (let i = 0; i < intervalDates.length; i++) {
      ...
      const itemsInInterval = records.filter(r => isWithinInterval(r.date, range))
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (type === 'PENGELUARAN') {
        ...
        if (!balance || balance.amount < input.amount) {
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message: `Saldo tidak mencukupi. Saldo saat ini ${formatIDR(balance?.amount ?? 0)}.`,
          })
        }
