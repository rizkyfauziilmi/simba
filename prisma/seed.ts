...
async function createAdmin(amount: number) {
    ...
    for (let i = 0; i < amount; i++) {
        ...
        if (!response.ok) {
            ...
            bar.stop();
            throw new Error(
                `Failed to create admin user: ${response.statusText}`,
            );
        }
...
async function createClasses(amount: number) {
    ...
    for (let i = 0; i < amount; i++) {
        const classData = await db.class.create({
            data: {
                namaKelas: `Kelas ${i + 1}`,
                tingkat: faker.helpers.arrayElement(
                    Object.values(StudentGrade),
                ),
                ruang: `R${i + 1}`,
                isLast: i === amount - 1,
                status: "AKTIF",
...
async function createTeachers(amount: number, classIds: string[]) {
    ...
    for (let i = 0; i < amount; i++) {
        ...
        if (!response.ok) {
            ...
            bar.stop();
            throw new Error(
                `Failed to create teacher user: ${response.statusText}`,
            );
        }
...
async function createStudents(amount: number, classIds: string[]) {
    ...
    for (let i = 0; i < amount; i++) {
        ...
        if (!response.ok) {
            ...
            bar.stop();
            throw new Error(
                `Failed to create student user: ${response.statusText}`,
            );
        }
...
async function createClassSchedules(
    ...
    const bar = new cliProgress.SingleBar({
        format: "🗓️ Creating ClassSchedules |{bar}| {value}/{total} ({percentage}%)",
        barCompleteChar: "\u2588",
        barIncompleteChar: "\u2591",
        hideCursor: true,
    });
    ...
    for (const _class of classes) {
        for (const hari of hariList) {
            ...
            for (let p = 0; p < periodsCount; p++) {
                ...
                const assignTeacher =
                    faker.datatype.boolean() && teachers.length > 0;
...
async function createFinanceRecords(extraAmount: number) {
    ...
    const bar = new cliProgress.SingleBar({
        format: "💰 Creating Finance Records |{bar}| {value}/{total} ({percentage}%)",
        barCompleteChar: "\u2588",
        barIncompleteChar: "\u2591",
        hideCursor: true,
    });
    ...
    if (existingBalance) {
        await db.schoolBalance.update({
            ...
            data: {
                amount: saldo,
                description:
                    "Saldo otomatis disinkronkan dari transaksi keuangan.",
            },
        });
    } else {
