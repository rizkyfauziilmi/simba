...
export function EditSubjectForm() {
    ...
    const { data: activeTeachers, isPending: isPendingActiveTeachers } =
        useQuery(trpc.teacher.getActiveTeachers.queryOptions());
    ...
    const initialSchedules =
        subject?.schedules?.map((schedule) => ({
            ...
            guruPengampuId: schedule.guruPengampuId ?? undefined,
        })) ?? [];
    ...
    const form = useForm<UpdateSubjectSchema>({
        ...
        defaultValues: {
            id: subject?.id,
            nama: subject?.nama,
            deskripsi: subject?.deskripsi ?? undefined,
            schedules: initialSchedules,
        },
    });

    const updateMapelMutationOptions =
        trpc.subject.updateSubject.mutationOptions({
            onError: (error) => {
    ...
    const updateMapelMutation = useMutation(updateMapelMutationOptions);

    function onSubmit(data: UpdateSubjectSchema) {
        updateMapelMutation.mutate(data);
    }

    const isLoading =
        updateMapelMutation.isPending || form.formState.isSubmitting;
                    ...
                    render={({ field }) => (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Anda dapat menambahkan beberapa jadwal
                                    dengan waktu yang berbeda. Pastikan untuk
                                    memilih kelas dan guru pengampu yang sesuai
                                    untuk setiap jadwal.
                                </CardDescription>
                                <FormMessage />
                                <CardAction>
